<ul class="mb-4 flex flex-wrap text-sm font-medium text-center text-gray-500 dark:text-gray-400">
  <li class="mr-2">
    <a href="{{ route('users.show', ['name' => $user->name]) }}"
      class="{{ $hasArticles ? 'btn-primary' : 'hover:bg-slate-200 dark:hover:bg-dark-2 py-2 px-4 rounded-lg' }} inline-block"
      aria-current="page">happy</a>
  </li>
  <li class="mr-2">
    <a href="{{ route('users.likes', ['name' => $user->name]) }}"
      class="{{ $hasLikes ? 'btn-primary' : 'hover:bg-slate-200 dark:hover:bg-dark-2 py-2 px-4 rounded-lg' }} inline-block">liked</a>
  </li>
