<template>
    <div class="relative">
        <button
            @click="show = !show"
            class="block text-white bg-primary rounded text-sm px-5 py-2.5 text-center"
        >
            ログイン
        </button>
        <Modal v-if="show" @close="show = !show">
            <template slot="header">コースの削除</template>
            <div class="w-full flex flex-col justify-center">
                <div class="main-body-content py-0">
                    <div class="card-body text-center">
                        <a
                            href="{{ route('login.{provider}', ['provider' => 'google']) }}"
                            class="google-btn flex items-center bg-white px-5 py-2.5 rounded"
                            ><svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="24"
                                height="24"
                                fill-rule="evenodd"
                                viewBox="0 0 24 24"
                            >
                                <path
                                    class="icon_svg-fill_as_stroke"
                                    d="M20.16 12.193c0-.603-.054-1.182-.155-1.739H12v3.288h4.575a3.91 3.91 0 0 1-1.696 2.565v2.133h2.747c1.607-1.48 2.535-3.659 2.535-6.248z"
                                    fill="#4285f4"
                                ></path>
                                <path
                                    class="icon_svg-fill_as_stroke"
                                    d="M12 20.5c2.295 0 4.219-.761 5.625-2.059l-2.747-2.133c-.761.51-1.735.811-2.878.811-2.214 0-4.088-1.495-4.756-3.504h-2.84v2.202A8.497 8.497 0 0 0 12 20.5z"
                                    fill="#34a853"
                                ></path>
                                <path
                                    class="icon_svg-fill_as_stroke"
                                    d="M7.244 13.615A5.11 5.11 0 0 1 6.977 12a5.11 5.11 0 0 1 .267-1.615V8.183h-2.84C3.828 9.33 3.5 10.628 3.5 12s.328 2.67.904 3.817l2.84-2.202z"
                                    fill="#fbbc05"
                                ></path>
                                <path
                                    class="icon_svg-fill_as_stroke"
                                    d="M12 6.881c1.248 0 2.368.429 3.249 1.271l2.438-2.438C16.215 4.342 14.291 3.5 12 3.5a8.497 8.497 0 0 0-7.596 4.683l2.84 2.202C7.912 8.376 9.786 6.881 12 6.881z"
                                    fill="#ea4335"
                                ></path>
                            </svg>
                            <span class="ml-4 text-lg">Googleでログイン</span>
                        </a>
                        <a
                            href="{{ route('login.{provider}', ['provider' => 'google']) }}"
                            class="google-btn mt-4 flex items-center bg-white px-5 py-2.5 rounded"
                            ><svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="24"
                                height="24"
                                fill-rule="evenodd"
                                viewBox="0 0 24 24"
                            >
                                <path
                                    class="icon_svg-fill_as_stroke"
                                    d="M20.16 12.193c0-.603-.054-1.182-.155-1.739H12v3.288h4.575a3.91 3.91 0 0 1-1.696 2.565v2.133h2.747c1.607-1.48 2.535-3.659 2.535-6.248z"
                                    fill="#4285f4"
                                ></path>
                                <path
                                    class="icon_svg-fill_as_stroke"
                                    d="M12 20.5c2.295 0 4.219-.761 5.625-2.059l-2.747-2.133c-.761.51-1.735.811-2.878.811-2.214 0-4.088-1.495-4.756-3.504h-2.84v2.202A8.497 8.497 0 0 0 12 20.5z"
                                    fill="#34a853"
                                ></path>
                                <path
                                    class="icon_svg-fill_as_stroke"
                                    d="M7.244 13.615A5.11 5.11 0 0 1 6.977 12a5.11 5.11 0 0 1 .267-1.615V8.183h-2.84C3.828 9.33 3.5 10.628 3.5 12s.328 2.67.904 3.817l2.84-2.202z"
                                    fill="#fbbc05"
                                ></path>
                                <path
                                    class="icon_svg-fill_as_stroke"
                                    d="M12 6.881c1.248 0 2.368.429 3.249 1.271l2.438-2.438C16.215 4.342 14.291 3.5 12 3.5a8.497 8.497 0 0 0-7.596 4.683l2.84 2.202C7.912 8.376 9.786 6.881 12 6.881z"
                                    fill="#ea4335"
                                ></path>
                            </svg>
                            <span class="ml-4 text-lg">Googleでログイン</span>
                        </a>
                    </div>
                </div>
            </div>
        </Modal>
    </div>
</template>
<script>
import Modal from "../Modal.vue";
export default {
    data: () => ({
        form: {
            email: "",
            password: "",
        },
        show: false,
    }),
    components: {
        Modal,
    },
    methods: {
        async login() {
            await this.$store.dispatch("user/login", this.form);
        },
    },
};
</script>
<style lang="scss" scoped>
.bg {
    @apply absolute w-screen h-screen top-0 right-0 left-0 bottom-0 m-0 p-0 z-10;
    background: rgba(#000000, 0.3);
}
.modal {
    @apply absolute top-0 left-0 ml-10 z-50 px-5 py-7 rounded shadow-lg overflow-y-auto cursor-default;
    background-color: var(--bg-secondary);
    @screen lg {
        width: 400px;
        max-height: 500px;
    }
}
.google-btn {
    border: 1px solid #dee0e1;
}
</style>
